...
class CreateChallengeScreen extends React.Component {
    ...
    addQuestion = () => {
        const condition1 = this.state.answer !== '' && Number(this.state.answer) !== NaN && (Number(this.state.answer) >= 0 && Number(this.state.answer) <= 3);
        const condition2 = this.state.prompt === '' || this.state.choice0 === '' || this.state.choice1 === '' || this.state.choice2 === '' || this.state.choice3 === '';
        ...
        if (this.state.index === 9) {
            ...
            Alert.alert('Fill in all required content before adding another question!')
            return false;
        }
        else if (!condition1) {
            Alert.alert('Correct answer should be a number between 0-3! (0=A, 1=B, 2=C, 3=D)')
            return false;
        }
        else {
            let q = {
                prompt: this.state.prompt,
                choices: [this.state.choice0, this.state.choice1, this.state.choice2, this.state.choice3],
    ...
    submit = () => {
        ...
        if (submit === true) {
            ...
            Alert.alert('Submitted ' + this.state.userQuestions.length + ' questions!')
            let tmpInd = 0;
            for (element in this.state.userQuestions) {
                console.log('Submitted')
                console.log('tmpInd= ' + tmpInd + ': prompt= ' + this.state.userQuestions[tmpInd].prompt + '; ' + this.state.userQuestions[tmpInd].choices + '; ' + this.state.userQuestions[tmpInd].answer)
                tmpInd++;
            }
        }
        else {
        ...
        firebase.firestore().collection('questions').doc(this.state.category).set({});
        firebase.firestore().collection('user categories').doc(this.state.category).set({});


        const ref = firebase.firestore().collection('questions').doc(this.state.category).collection('questions');
        ...
        this.unsubscribe2 = userRef.collection('my challenges').doc(this.state.category).set({})
        ...
        return (
            <View style={{ flex: 1, alignItems: 'center' }}>
                ...
                <SafeAreaView style={{ alignItems: 'center' }}>
                    <Text style={{ color: 'blue', paddingBottom: 20 }}>Create Challenge</Text>
                                ...
                                return (
                                    <View
                                        ...
                                        <View style={{ flexDirection: 'row' }}>
                                            <Text style={{ color: 'white', fontSize: 15 }}>Prompt: </Text>
                                            ...
                                            <TextInput
                                                ...
                                                value={this.state.userQuestions[index].prompt === '' ? this.state.prompt : this.state.userQuestions[index].prompt}
                                            />
                                        </View>
                                        ...
                                        <View style={{ flexDirection: 'row' }}>
                                            ...
                                            <TextInput
                                                ...
                                                placeholderTextColor='blue'
                                                style={{ color: 'white', width: 100 }}
                                                onChangeText={text => this.setState({ answer: Number(text) })}
                                                autoCorrect={false}
                                                defaultValue={this.state.userQuestions[index].answer.toString()}
                                            />
                                        </View>
